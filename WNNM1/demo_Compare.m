...
for i = 1:numImages
    if strcmp(denoising_type, 'CleanImage')
        ...
        for ch = 1:channel
            Parameters.channels_noise_std(ch) = EstimateNoise(noise_image(:, :, ch), Parameters.patsize); 
        end
    ...
    denoised_image = zeros(size(noise_image)); 
    for ch = 1:channel
        Parameters.nSig = Parameters.channels_noise_std(ch); 
        Parameters.ch = ch; 
        denoised_image(:, :, ch) = WNNM_DeNoising(noise_image(:, :, ch), ref_image(:, :, ch), Parameters); 
    end
    if Parameters.bEvaluate
...
if Parameters.bEvaluate
    fprintf('The average PSNR = %2.4f, SSIM = %2.4f. \n', mean(Parameters.PSNR), mean(Parameters.SSIM)); 
end
fprintf('Finish! \n'); 
